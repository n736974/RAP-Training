...
INCLUDE TYPE zvend_unm_navbh AS data.
...
ls_failed-%msg = new_message( id = 'ZRAP_NAVBH'
number = 000
...
SELECT MAX( sord ) FROM zvend_unm_navbh INTO @DATA(lv_max_sord).
...
SELECT SINGLE * FROM zvend_unm_navbh WHERE sord = @<lfs_new_data>-Sord
...
SELECT * FROM zvend_unm_navbh WHERE sord = @<lfs_keys>-Sord INTO TABLE @DATA(lt_result_tmp) .
...
LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
SELECT SINGLE * FROM zvend_unm_navbh WHERE sord = @<lfs_keys>-Sord INTO @DATA(ls_result_tmp) .
...
CLASS lsc_ZI_VEND_UNM_NAVBH DEFINITION INHERITING FROM cl_abap_behavior_saver.
PROTECTED SECTION.
...
CLASS lsc_ZI_VEND_UNM_NAVBH IMPLEMENTATION.
